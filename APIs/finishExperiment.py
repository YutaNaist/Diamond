...
class FinishExperiment:
    ...
    def set_API_Key_Google_Drive(self, str_Directory_API_Key_Drive,
                                 str_Directory_Setting_Yaml):
    ...
    def finishExperiment(self):
        ...
        try:
            if self._moveExperimentResults():
                ...
                dictReturnMsg[
                    "message"] = "The folder of ID: {} is already exists.".format(
                        self.experiment_ID)
        except shutil.Error:
            pass
            ...
            dictReturnMsg["status"] = False
            dictReturnMsg[
                "message"] = "The filename of ({}) has been already used. Please change filename!".format(
    ...
    def _moveExperimentResults(self):
        ...
        self._moveExperimentResultsMain(self.str_Share_Directory,
    ...
    def _moveExperimentResultsMain(self, str_Share_Directory,
                                   str_Experiment_Directory):
    ...
    def _makeUserInfoJson(self):
        ...
        proposal_filename = self.str_Experiment_Directory + self.experiment_ID + ".json_prop"
    ...
    def _makeMetaDataJsons(self):
        metaDirectory = self.str_Experiment_Directory + self.experiment_ID + "/metadata/"
        ...
        for i, file in enumerate(self.list_File_Names):
            ...
            json.dump(self.listDictMetaDatas[i],
                      open(jsonFileName, "w", encoding='utf-8'),
                      indent=4,
                      ensure_ascii=False)

    def _makeFilenamesLists(self):
        json.dump(self.list_File_Names,
                  open(self.str_Experiment_Directory + "filenames.json",
                       "w",
                       encoding='utf-8'),
                  indent=4,
                  ensure_ascii=False)
    ...
    def _save_Experiment_information(self):
        experiment_Information_File_Name = self.str_Experiment_Directory + "experiment_information.json"
        json.dump(self.dict_Experiment_Information,
                  open(experiment_Information_File_Name, "w",
                       encoding='utf-8'),
                  indent=4,
                  ensure_ascii=False)
    ...
    def _up_load_to_Google(self):
        if self.is_Share_With_Google is False:
            ...
            google_Drive_Handler.upload_Folder(up_Load_Directory,
                                               up_Load_Directory_Name)
            return True
