...
class GoogleSpreadSheetHandler:
    def __init__(
        self, str_Authorize_Json: str = "", logger: logging.Logger = None
    ) -> None:
        if logger is None:
    ...
    def load_All_Value_From_Spread_Sheet(self, str_URL_Spread_Sheet: str = "") -> list:
        try:
    ...
    def update_Spread_Sheet_IDs_From_Sheet(self, str_URL_Spread_Sheet: str) -> bool:
        sheet = self.client.open_by_url(str_URL_Spread_Sheet)
            ...
            try:
                ...
                for i in range(len(data) - 1):
                    if data[i + 1][1] != "" and data[i + 1][2] != "":
                        ...
                        self.list_URL_SpreadSheet_Brows.append(data[i + 1][1])
                ...
                # self.logger.warning("APIError in update IDs")
                time.sleep(10)
                return self.update_Spread_Sheet_IDs_From_Sheet(str_URL_Spread_Sheet)
    ...
    def overwrite_2D_Array_To_Spread_Sheet(
        self, str_URL_Spread_Sheet: str, str_Save_Range: str, list_All_Value: list
    ) -> None:
        try:
            sheet = self.client.open_by_URL(str_URL_Spread_Sheet)
            worksheet = sheet.get_worksheet(0)
            ...
            self.logger.warning("APIError in overwrite 2d array to spread sheet")
            # print(e)
            time.sleep(10)
            self.overwrite_2D_Array_To_Spread_Sheet(
                self, str_URL_Spread_Sheet, str_Save_Range, list_All_Value
            )

    def overwrite_All_Value_To_Spread_Sheet(
        self, str_URL_Spread_Sheet: str, list_All_Value: list
    ) -> None:
        try:
            sheet = self.client.open_by_url(str_URL_Spread_Sheet)
            worksheet = sheet.get_worksheet(0)
    ...
    def overwrite_All_Value_To_Spread_Sheet_Brows(
        ...
        try:
            ...
            time.sleep(10)
            # print("resend")
            self.overwrite_All_Value_To_Spread_Sheet_Brows(index, list_All_Value)

    def overwrite_ID_Value_To_Spread_Sheet(
        self, index: int, list_All_ID: list, int_Col: int
    ) -> None:
        ...
        try:
            ...
            worksheet_range = (
                self.toAlpha(int_Col) + "2:" + self.toAlpha(int_Col) + str(row_last + 2)
            )
            ...
            time.sleep(10)
            # print("resend")
            self.overwrite_ID_Value_To_Spread_Sheet(index, list_All_ID, int_Col)
